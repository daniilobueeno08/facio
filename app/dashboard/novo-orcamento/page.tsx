          ...
          <div style={{ background:C.bg, borderRadius:16, border:`1px solid ${C.border}`, padding:16, marginBottom:16, overflow:"hidden" }}>
            ...
            {catalog.map((c) => (
              <div key={c.id} style={{ display:"grid", gridTemplateColumns:"1.4fr 0.85fr 0.85fr 0.85fr", gap:6, alignItems:"center", padding:"8px 0", borderBottom:`1px solid ${C.surface}` }}>
                ...
                <button type="button" onClick={() => addFromCatalog(c.nome, c.valor_higienizacao, "Higienização")} style={{ background:C.pale, border:"1px solid rgba(99,153,34,.3)", borderRadius:8, padding:"6px 2px", fontSize:11, fontWeight:600, color:C.dark, cursor:"pointer" }}>{c.valor_higienizacao.toFixed(0)}</button>
                {c.valor_blindagem!=null?(<button type="button" onClick={() => addFromCatalog(c.nome, c.valor_blindagem!, "Blindagem")} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 2px", fontSize:11, fontWeight:600, color:C.text, cursor:"pointer" }}>{c.valor_blindagem.toFixed(0)}</button>):(<span style={{ textAlign:"center", fontSize:11, color:"#D1D5DB" }}>—</span>)}
                {c.valor_combo!=null?(<button type="button" onClick={() => addFromCatalog(c.nome, c.valor_combo!, "Combo")} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 2px", fontSize:11, fontWeight:600, color:C.text, cursor:"pointer" }}>{c.valor_combo.toFixed(0)}</button>):(<span style={{ textAlign:"center", fontSize:11, color:"#D1D5DB" }}>—</span>)}
              </div>
            ))}
        ...
        <div style={{ background:C.bg, borderRadius:16, border:`1px solid ${C.border}`, padding:16, marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:formaPagamento==="crediario"?12:0 }}>
            {(["avista","crediario"] as const).map((op) => (
              <button key={op} type="button" onClick={() => setFormaPagamento(op)} style={{ flex:1, padding:"12px 0", borderRadius:12, border:`2px solid ${formaPagamento===op?C.primary:C.border}`, background:formaPagamento===op?C.pale:C.surface, color:formaPagamento===op?C.dark:C.muted, fontWeight:600, fontSize:13, fontFamily:"var(--font-display)", cursor:"pointer" }}>
          ...
          {formaPagamento==="crediario" && (
            <div>
              ...
              <input type="date" value={dataVencimento} onChange={(e) => setDataVencimento(e.target.value)} style={{ ...inp, fontSize:13, padding:"10px 12px" }} />
              <p style={{ fontSize:11, color:"#92400E", marginTop:6 }}>Ao aprovar, este orçamento irá automaticamente para o Crediário.</p>
            </div>
          )}
        </div>
